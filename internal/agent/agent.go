// Package agent is the core orchestration layer for PuppyPatch AI agents.
//
...
var userAgent = fmt.Sprintf("Charm-PuppyPatch/%s (https://charm.land/puppypatch)", version.Version)
...
type SessionAgentCall struct {
	...
	// reliable completion contract (e.g. `puppypatch run` against a
	...
	// event, so non-interactive clients (e.g. `puppypatch run`) don't
...
// RunID, e.g. `puppypatch run`, would otherwise hang). Uncanceled calls without
...
// that RunID: `puppypatch run` ignores live message events and exits only on a
...
// so callers waiting on those RunIDs (e.g. `puppypatch run`) are not left
...
// caller waiting on RunComplete (e.g. `puppypatch run` with a RunID) always
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if call.Accepted != nil {
		...
		if a.canceledBySeq(call.SessionID, call.Accepted.seq) {
			...
			// RunComplete for this RunID (e.g. `puppypatch run`, which
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "puppypatch auth" to re-authenticate.`)
