...
import (
	...
	"github.com/puppypatch/puppypatch/internal/client"
	"github.com/puppypatch/puppypatch/internal/config"
	"github.com/puppypatch/puppypatch/internal/db"
	"github.com/puppypatch/puppypatch/internal/event"
	"github.com/puppypatch/puppypatch/internal/lock"
	crushlog "github.com/puppypatch/puppypatch/internal/log"
	"github.com/puppypatch/puppypatch/internal/projects"
	"github.com/puppypatch/puppypatch/internal/proto"
	"github.com/puppypatch/puppypatch/internal/server"
	...
	ui "github.com/puppypatch/puppypatch/internal/ui/model"
	"github.com/puppypatch/puppypatch/internal/version"
	"github.com/puppypatch/puppypatch/internal/workspace"
...
func init() {
	...
	rootCmd.PersistentFlags().StringP("data-dir", "D", "", "Custom puppypatch data directory")
	rootCmd.PersistentFlags().BoolP("debug", "d", false, "Debug")
	rootCmd.PersistentFlags().StringVarP(&clientHost, "host", "H", server.DefaultHost(), "Connect to a specific puppypatch server host (for advanced users)")
...
puppypatch run "Guess my 5 favorite Pokémon"

# Run a non-interactively with pipes and redirection
cat README.md | puppypatch run "make this more glamorous" > GLAMOROUS_README.md

# Run with debug logging in a specific directory
puppypatch --debug --cwd /path/to/project

# Run in yolo mode (auto-accept all permissions; use with care)
puppypatch --yolo

# Run with custom data directory
puppypatch --data-dir /path/to/custom/.puppypatch

# Continue a previous session
puppypatch --session {session-id}
  ...
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := program.Run(); err != nil {
			...
			return errors.New("PuppyPatch crashed. If metrics are enabled, we were notified about it. If you'd like to report it, please copy the stacktrace above and open an issue at https://github.com/puppypatch/puppypatch/issues/new?template=bug.yml") //nolint:staticcheck
		}
		return nil
	},
}
...
func setupLocalWorkspace(cmd *cobra.Command) (workspace.Workspace, func(), error) {
	...
	logFile := filepath.Join(cfg.Options.DataDirectory, "logs", "puppypatch.log")
...
func connectToServer(cmd *cobra.Command) (*client.Client, *proto.Workspace, func(), error) {
	...
	if ws.Config != nil {
		logFile := filepath.Join(ws.Config.Options.DataDirectory, "logs", "puppypatch.log")
		crushlog.Setup(logFile, debug)
	}
...
func ensureServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	debug, _ := cmd.Flags().GetBool("debug")
	logFile := filepath.Join(config.GlobalCacheDir(), "server-"+safeHostName(hostURL), "puppypatch.log")
	crushlog.Setup(logFile, debug)
	...
	case "unix", "npipe":
		...
		if needsStart {
			if err := spawnAndWaitReady(cmd, hostURL); err != nil {
				return fmt.Errorf("failed to initialize puppypatch server: %v", err)
			}
			return nil
		}
...
// $XDG_CACHE_HOME/puppypatch/server-<safeHost>/start.lock.
//
...
func startDetachedServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	if err := c.Start(); err != nil {
		return fmt.Errorf("failed to start puppypatch server: %v", err)
	}

	if err := c.Process.Release(); err != nil {
		return fmt.Errorf("failed to detach puppypatch server process: %v", err)
	}
