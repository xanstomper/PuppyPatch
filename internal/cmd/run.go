...
import (
	...
	"github.com/puppypatch/puppypatch/internal/format"
...
curl https://charm.land | puppypatch run "Summarize this website"

# Read from a file
puppypatch run "What is this code doing?" <<< prrr.go

# Redirect output to a file
puppypatch run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
puppypatch run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
puppypatch run --verbose "Generate a README for this project"

# Continue a previous session
puppypatch run --session {session-id} "Follow up on your last response"

# Continue the most recent session
puppypatch run --continue "Follow up on your last response"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if useClientServer() {
			...
			if !ws.Config.IsConfigured() {
				return fmt.Errorf("no providers configured - please run 'puppypatch' to set up a provider interactively")
			}
...
// streaming SSE events into a final, complete stdout for `puppypatch run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `puppypatch run` exited
...
// prefix returned by puppypatch session list.
