...
// puppypatch process from racing the storage.
...
func Connect(ctx context.Context, dataDir string, opts ...ConnectOption) (*sql.DB, error) {
	...
	dbPath := filepath.Join(dataDir, "puppypatch.db")
	...
	// puppypatch process on the same SQLite file. The lock is released when
	...
	// local-mode invocations do not refuse a second puppypatch against the
