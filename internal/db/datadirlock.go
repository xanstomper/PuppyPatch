...
// already in use by another puppypatch process.
var ErrDataDirLocked = errors.New("data directory already in use by another puppypatch process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to puppypatch.db so users can `ls` and find it.
const dataDirLockFile = "puppypatch.lock"
...
// {dataDir}/puppypatch.lock. If the lock is already held by another
