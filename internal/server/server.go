...
import (
	...
	_ "github.com/puppypatch/puppypatch/internal/swagger"
...
// socketDir returns the directory used for the PuppyPatch Unix socket.
...
// returned by [socketDir] and is named puppypatch-<uid>.sock, falling
// back to puppypatch.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/puppypatch-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "puppypatch.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("puppypatch-%s.sock", usr.Uid)
	}
...
// Server represents a PuppyPatch server bound to a specific address.
type Server struct {
