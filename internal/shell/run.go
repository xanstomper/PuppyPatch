...
type RunOptions struct {
	...
	// the PuppyPatch process cwd — hooks and the bash tool have different
...
// PuppyPatch handler stack. Shared by the stateless [Run] entrypoint and the
...
// standard PuppyPatch middleware chain (builtins, script dispatch, block list)
...
// when sourcing framework files can send SIGINT/SIGTERM to PuppyPatch's process
...
// regardless of the caller's environment because PuppyPatch shells are never
...
//  1. builtins first (so PuppyPatch's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized PuppyPatch
